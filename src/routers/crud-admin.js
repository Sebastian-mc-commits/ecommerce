...
import { authenticateAdmin, authenticateSuperAdmin } from "../lib/middleware/authentication.js";
import * as product from "../services/product.service.js";
import { unsetUserToAdmin, userToAdmin } from "../services/user.service.js";
...
router.put("/setUserToAdmin/:id", authenticateAdmin, async (req, res) => {
    try {
        const adminId = req.session.user._id;
        const response = await userToAdmin(adminId, req.params.id);
        // res.redirect("/crud-admin");
        res.json(response);
    }
    catch (err) {
...
router.put("/unsetToAdmin/:id", authenticateSuperAdmin, async (req, res) => {
    try {
        const adminId = req.session.user._id;
        const response = await unsetUserToAdmin(adminId, req.params.id);
        // res.redirect("/crud-admin");
        res.json(response);
    }
    catch (err) {
